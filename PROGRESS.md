...
- [x] **Elegir qué categorías se muestran en la home** — pedido del usuario 2026-08-04 ("por categoría, los que van aparecen en la página de inicio"). Migración `supabase/009_categorias_visibilidad.sql`: tabla `categorias_config` (categoria, visible_web), sembrada con las 25 categorías, todas visibles por default (no cambia nada hasta que se desmarque alguna).
  ...
  - El usuario después usó el panel por su cuenta para curetar de verdad qué categorías se ven en la web (dejó visibles Panes, Facturas, Tortas, Sandwiches, Dulces, Vinos y Copetín; el resto quedó oculto) — es una elección real del negocio, no tocar sin que lo pida.
- [x] **Módulo Categorías: crear/configurar categorías con imagen propia** — pedido del usuario 2026-08-04 ("agregar un ítem para agregar o configurar categoría, donde se pueda subir una imagen, así también aparece en la página de inicio"). Migración `supabase/010_categorias_imagen.sql`: agrega `categorias_config.imagen_url`.
  - Nuevo módulo `/admin/categorias` (mismo nivel de permiso que Productos: superadmin + encargado_sucursal) reemplaza al panel de "categorías visibles" que vivía adentro de Productos (se borró `categorias-visibilidad-form.tsx` y la acción vieja, quedaban duplicados). Desde ahí se pueden **crear categorías nuevas** (no solo las 25 originales) y subirles una foto — se guarda en el mismo bucket de Storage que las imágenes de producto, bajo `categorias/`.
  - El banner de categoría (`src/components/categoria-banner.tsx`, componente compartido nuevo) ahora muestra la foto de fondo con degradé oscuro para que se lea el texto, si la categoría tiene una cargada; si no, sigue mostrando el ícono genérico de siempre. Se aplicó en Productos y Stock (usan el componente compartido) y en la home pública (mantiene su propio estilo con tipografía Playfair, pero con el mismo tratamiento de foto).
  - El desplegable de categoría al crear/editar un producto (antes una lista fija en el código) ahora sale de la base (`categorias_config`), así una categoría nueva creada en `/admin/categorias` aparece de inmediato como opción en Productos.
  - De paso corregí un bug viejo: a "Personal" le faltaba "reportes" en la lista de módulos que realmente se guardan (el checkbox estaba pero no hacía nada) — ahora si tildan Reportes, funciona.
  - ~~Pendiente: falta que el usuario corra `supabase/010_categorias_imagen.sql`~~ — corrida.
- [x] **Horarios del personal, reporte manual de faltantes y permiso por categoría** — pedido del usuario 2026-08-04 ("que al superadmin le avise quiénes están trabajando en qué horario... que el cajero o encargado pueda poner qué faltante de stock tiene... en qué categorías pueden los empleados reportar faltantes"). Migración `supabase/011_horarios_y_faltantes.sql`.
  - **Nuevo módulo Horarios** (`/admin/horarios`, visible para todo el staff con el módulo habilitado — carga y borrado solo superadmin/encargado_sucursal, vía RLS y `requireRol`). Tabla nueva `turnos_personal` (empleado, sucursal, fecha, hora inicio/fin, notas). Filtro por período (Hoy / Esta semana / Este mes, `src/app/admin/horarios/horarios-filtros.tsx`) — a diferencia de Reportes (que mira hacia atrás), acá el rango es hacia adelante/actual: semana de lunes a domingo, mes calendario actual. Turnos agrupados por día, ordenados por hora (`turnos-list.tsx`). Agregado a `MODULOS_NAV`/`BOTON_GRADIENTE` (`src/lib/admin-modulos.ts`, ícono reloj) y a `MODULOS_ACCESO`/`MODULOS` de Personal para poder habilitarlo por empleado.
  - **Reporte manual de faltante de stock**: nuevo formulario en `/admin/caja` (`reportar-faltante-form.tsx`) — a propósito puesto ahí y no en Notificaciones, porque el pedido decía explícitamente "el cajero o encargado", y Caja es el único módulo al que tiene acceso la cuenta de cajera. Categoría (desplegable) + descripción libre; server action `reportarFaltanteStock` en `caja/actions.ts` valida que la categoría elegida tenga permitido el reporte antes de insertar. Genera notificación tipo `faltante_reportado` (nueva función `notificarFaltante` en `src/lib/notificaciones.ts`), visible al superadmin y al resto del staff de esa misma sucursal (mismo esquema de scoping que `stock_bajo`, sin necesidad de tocar `layout.tsx` ni `/admin/notificaciones` porque ya filtran genéricamente por tipo/sucursal).
  - **Permiso de reporte por categoría**: `categorias_config.permite_reportar_faltante` (default `true`), con checkbox nuevo tanto en "Nueva categoría" como en la edición de cada categoría (`/admin/categorias`). El desplegable de categorías en Caja solo lista las que tienen el permiso activo; si el superadmin lo destilda para una categoría, sus productos dejan de poder reportarse como faltantes (pero se puede seguir vendiendo/gestionando stock normalmente, esto solo afecta el reporte manual).
  - Ícono de la notificación `faltante_reportado`: mismo símbolo que `stock_bajo` (triángulo de alerta) pero en naranja para distinguir "lo reportó un empleado a mano" de "lo detectó el sistema por umbral".
  - Probado en el navegador logueado como superadmin: la página de Horarios carga con el desplegable de empleados/sucursales reales y el filtro de período; el formulario de faltantes en Caja no rompe la página aunque la migración todavía no esté corrida (query falla en silencio, el formulario simplemente no se muestra); `/admin/categorias` muestra el nuevo checkbox sin romper el listado existente.
  - **Pendiente**: falta que el usuario corra `supabase/011_horarios_y_faltantes.sql`. Hasta entonces, Horarios muestra el error "Could not find the table 'public.turnos_personal'" en vez de la lista (no rompe la página), y el formulario de reporte de faltantes en Caja queda oculto (no aparece ninguna categoría para elegir).
