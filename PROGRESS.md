...
- [x] **Panel mobile-friendly, PWA instalable, cuenta de cajera y alerta de ventas al superadmin** — pedido del usuario 2026-08-03/04.
  ...
  - **Pendiente**: falta que el usuario corra `supabase/008_notificacion_venta.sql`. Probé el flujo completo (cajera carga un "Ingreso" de $1500 con descripción "Factura #001") y confirmé que el movimiento de caja se registra bien, pero la notificación al superadmin no se generó porque el `check constraint` todavía no tiene el tipo nuevo — es un fallo silencioso a propósito (no quería que un error de notificación le impida a la cajera cobrar), pero hay que correr la migración para que la alerta funcione de verdad.
