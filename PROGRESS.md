...
- [x] **Módulo Categorías: crear/configurar categorías con imagen propia** — pedido del usuario 2026-08-04 ("agregar un ítem para agregar o configurar categoría, donde se pueda subir una imagen, así también aparece en la página de inicio"). Migración `supabase/010_categorias_imagen.sql`: agrega `categorias_config.imagen_url`.
  ...
  - **Pendiente**: falta que el usuario corra `supabase/010_categorias_imagen.sql`. Mientras tanto, crear una categoría nueva o subir una imagen da un error claro (no rompe la página) — probado, así se ve el mensaje: "Could not find the 'imagen_url' column...". Más urgente que otras migraciones pendientes porque hasta que se corra, el desplegable de categoría al cargar/editar un producto queda vacío.
