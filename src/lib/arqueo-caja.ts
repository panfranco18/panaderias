...
export async function calcularArqueo(supabase: Supabase, sucursalId: string, fecha: string) {
  ...
  const [{ data: ventas }, { data: movimientos }, cierreXResultado] = await Promise.all([
    ...
    supabase
      ...
      .gte("fecha", inicio)
      .lt("fecha", fin),
    obtenerCierreXDeHoy(supabase, sucursalId, fecha),
  ]);
  ...
  for (const v of ventas ?? []) {
    ...
    porMetodo.set(metodo, (porMetodo.get(metodo) ?? 0) + Number(v.total));
  }
  ...
  const ventasNegro = grupos.get("venta_1")?.total ?? 0;
  const ventasRegistradas = grupos.get("venta_deleite")?.total ?? 0;
  const ventasSinClasificar = grupos.get("sin_clasificar")?.total ?? 0;

  const comparacion = {
    ventas: {
      negro: ventasNegro,
      registradas: ventasRegistradas,
      sinClasificar: ventasSinClasificar,
      total: ventasNegro + ventasRegistradas + ventasSinClasificar,
    },
    valores: {
      posnet: porMetodo.get("posnet") ?? 0,
      tarjeta: porMetodo.get("tarjeta") ?? 0,
      mercadopago: porMetodo.get("mercadopago") ?? 0,
      transferencias: porMetodo.get("transferencia") ?? 0,
      depositos,
      gastos,
    },
  };

  return { secciones, totalGeneral, comparacion };
}
...
async function obtenerCierreXDeHoy(supabase: Supabase, sucursalId: string, fecha: string) {
